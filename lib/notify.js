...
async function sendEmail({ to, subject, html }) {
  ...
  const from = process.env.NOTIFY_FROM_EMAIL || 'bookings@clubconcierge.com';
...
async function notifyBookingSuccess(user, { clubName, date, teeTime, numPlayers, inPreferredWindow }) {
  ...
  const smsText = [
    `⛳ Club Concierge booked your tee time!`,
    `Club: ${clubName}`,
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 32px; background: #faf7f2;">
        ...
        <p style="color: #555;">Your Club Concierge agent secured a tee time:</p>
      ...
      <div style="padding: 16px; text-align:center; color:#aaa; font-size:12px; background:#f0ece4;">
        Club Concierge · <a href="https://clubconcierge.com" style="color:#1f3b2d;">clubconcierge.com</a>
      </div>
    </div>`;
...
async function notifyBookingFailed(user, { reason, date }) {
  const smsText = `⚠️ Club Concierge could not book your tee time for ${date}.\nReason: ${reason}\n\nWe'll keep checking and alert you when a slot opens.`;
